import type { Game, PlayerScore, Scoreboard } from '../../steps/chess/streams/00-chess-game.stream'
import type { GameMove } from '../../steps/chess/streams/00-chess-game-move.stream'
...
export const generateGameScore = async (
  game: Game,
  moves: GameMove[],
): Promise<{ whiteScore: PlayerScore; blackScore: PlayerScore; scoreboard?: Scoreboard }> => {
  if (moves.length === 0) {
    return {
  ...
  const averageEval = evaluations.length > 0 ? evaluations.reduce((sum, node) => sum + node, 0) / evaluations.length : 0

  // Calculate average swing
  const averageSwing = swings.length > 0 ? swings.reduce((sum, swing) => sum + swing, 0) / swings.length : 0
...
export const generateScoreboard = (
  ...
  if (moves.length > 1) {
    ...
    if (maxSwing > 0.5) {
      // Only include if there was a significant swing
      decisiveMoment = {
