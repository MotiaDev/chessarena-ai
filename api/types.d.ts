...
declare module 'motia' {
  ...
  type Handlers = {
    ...
    'MovePiece': ApiRouteHandler<{ password: string; promote?: 'queen' | 'rook' | 'bishop' | 'knight'; from: string; to: string }, ApiResponse<200, { id: string; fen: string; turn: 'white' | 'black'; status: 'pending' | 'completed' | 'draw'; lastMove?: string[]; winner?: 'white' | 'black'; players: { white: { name: string; ai?: 'openai' | 'gemini' | 'claude' }; black: { name: string; ai?: 'openai' | 'gemini' | 'claude' } }; check: boolean }> | ApiResponse<400, { message: string }> | ApiResponse<404, { message: string }>, { topic: 'chess-game-moved'; data: { gameId: string; fenBefore: string } } | { topic: 'chess-game-ended'; data: { gameId: string } }>
